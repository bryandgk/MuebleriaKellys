						...
						if(searchAdmin.isEmpty()){
							...
							if(entradaAdmin){
								...
								if(idRoleAdmin.equals(searchAccess.get(0).getIdRol()) && searchAccess.get(0).getResource().equals("/facturas")){
									String query = "select from " + Pedido.class.getName();
									List<Pedido> listaPedido = (List<Pedido>) pm.newQuery(query).execute();
									req.setAttribute("listaPedido", listaPedido);
									String query2 = "select  from " + Facturar.class.getName();
									List<Facturar> listas = (List<Facturar>) pm.newQuery(query2).execute();
									req.setAttribute("listas", listas);
									req.getRequestDispatcher("/WEB-INF/Views/Factura/index.jsp").forward(req, resp);
								} else {
